...
router.get("/permenit/:mcbId", async (req, res) => {
    ...
    const realtimeCollection = process.env.REALTIME_COLLECTION || "monitoring_percobaan";

    try {
        // 1) Konversi input WIB → UTC
        const toUtc = (localISOString) => {
        ...
        // 3) Mapping data: UTC → WIB
        const data = snapshot.docs.map(doc => {
            const d = doc.data();
            d._id = doc.id;  
            if (d.timestamp && typeof d.timestamp.toDate === "function") {
                d.timestamp = d.timestamp.toDate().getTime();
            }
            return d;
        });

        // Menampilkan dalam bentuk timestamp jakarta
        //   const pad2 = n => String(n).padStart(2,"0");
        // const data = snapshot.docs.map(doc => {
        //   const d = doc.data();
        //   if (d.timestamp && typeof d.timestamp.toDate === "function") {
        //     const utc = d.timestamp.toDate();
        //     const wib = new Date(utc.getTime() + 7*3600*1000);
        //     d.timestamp = 
        //       `${wib.getFullYear()}-${pad2(wib.getMonth()+1)}-${pad2(wib.getDate())}`+
        //       `T${pad2(wib.getHours())}:${pad2(wib.getMinutes())}:${pad2(wib.getSeconds())}`;
        //   }
        //   return d;
        // });
        ...
        return res.json({ data });
            ...
            return res.status(500).json({ error: "Gagal mengambil data permenit" });
      }
});
